...
public class ReplicatorServlet extends SlingAllMethodsServlet {
    ...
    protected void doGet(SlingHttpServletRequest request, SlingHttpServletResponse response) throws IOException, ServletException {
        ...
        try {
            final Repository targetRepository = JcrUtils.getRepository("http://localhost:4552/crx/server");
            targetSession = targetRepository.login(new SimpleCredentials("admin", "admin".toCharArray()));

            targetSession.getWorkspace().importXML("/content", new ByteArrayInputStream(FAKE_TEST_NODE.getBytes()), ImportUUIDBehavior.IMPORT_UUID_COLLISION_REPLACE_EXISTING);
