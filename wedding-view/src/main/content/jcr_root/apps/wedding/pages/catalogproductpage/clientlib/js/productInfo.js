var PORTAL = (function (PORTAL, $) {
    ...
    PORTAL.modules.ProductInfo.selfSelector = ".product-info-model";

    PORTAL.modules.ProductInfo.init = function ($self) {

        console.log('Component: "ProductInfo"');

        $self.find(".video-item").each(function() {
            var item = $(this);
            var videoID = PORTAL.utils.getQueryParameterFromUrl(item.data("url"),"v");
            item.find("iframe").attr("src","https://www.youtube.com/embed/"+videoID);
        });


        $($self).find('#photo-grid-gallery img').each(function () {
            $(this).load(function () {
        ...
        $self.find('#photo-grid-gallery').magnificPopup({
            delegate: 'a',
            type: 'image',
