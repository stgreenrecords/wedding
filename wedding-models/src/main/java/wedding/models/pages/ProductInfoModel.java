...
import wedding.models.beans.ProductInfoProperty;
import wedding.models.beans.SellerInfo;
import wedding.models.beans.SimplePageBean;
import wedding.models.comparators.SellerComparator;
...
public class ProductInfoModel extends BaseModel {
    ...
    public static List<SellerInfo> parseSellerProperty(String[] sellerPropertyArray, Session session) {
        ...
        if (sellerPropertyArray != null) {
            for (String sellerProperty : sellerPropertyArray) {
                ...
                int rating = 0;
                try {
                    Node sellerProfileNode = session.getNode("/home/users/wedding/" + propertySellerID + "/profile");
    ...
    public List<String> getPhotosList() {
        ...
        Resource associatedDAMResource = getResourceResolver().getResource(associatedDAMPath + "/photos");
    ...
    public List<String> getVideoList() {
        ...
        String associatedDAMPath = getCurrentPage().getPath().replace("/content", "/content/dam");
        Resource associatedDAMResource = getResourceResolver().getResource(associatedDAMPath + "/videos");
        if (associatedDAMResource != null) {
            for (Resource resource : associatedDAMResource.getChildren()) {
                if (!resource.getName().equals(JcrConstants.JCR_CONTENT)) {
                    videoList.add(resource.getPath());
                }
            }
        }
        return videoList;
